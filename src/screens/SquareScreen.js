...
const reducer = (state, action) => {
    ...
    // action === {colorToChange: 'change_red' || 'change_green' || 'change_blue', amount: 15 || -15}
    const { type, payload } = action;
    switch (type) {
        case 'change_red':
            return state.red + payload > 255 || state.red + payload < 0 ? state : { ...state, red: state.red + payload };
        case 'change_green':
            return state.green + payload > 255 || state.green + payload < 0 ? state : { ...state, green: state.green + payload };
        case 'change_blue':
